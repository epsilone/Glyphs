...
- (void)showFacebookComposerWithMessage:(NSString*)message image:(UIImage*)image link:(NSString*)link
{
	...
	SLComposeViewController* slComposer = [SLComposeViewController composeViewControllerForServiceType:@"com.apple.social.facebook"];
...
{
	[[FacebookManager sharedLoginManager] logInWithReadPermissions:permissions handler:^( FBSDKLoginManagerLoginResult* result, NSError* error )
	 {
		 if( error )
		 {
...
{
	[[FacebookManager sharedLoginManager] logInWithPublishPermissions:permissions handler:^( FBSDKLoginManagerLoginResult* result, NSError* error )
	 {
		 if( error )
		 {
			 NSLog( @"session creation error: %@ userInfo: %@", error, error.userInfo ? error.userInfo : @"no userInfo" );
